...
outputFileName <- sub("matrix", "Deseq2_res", countsFileName)
...
if (
    file.exists(countsFileName) && file.exists(coldataFileName)
) {
    message("========================DEseq2==========================")
    message("\n*****Working Dir")
    message(getwd())
...
colData$condition <- factor(colData$condition, 
                            levels = unique(colData$condition))
colData$colNames <- factor(colData$colNames, 
                           levels = unique(colData$colNames))
rawCounts <-
    data.table::fread(countsFileName, data.table = F) %>%
...
counts.rlog <- as_tibble(assay(rld), rownames = "ID")
...
fig.pca <-
    ...
    filter(
        condition %in% unique(unlist(strsplit(groups.AvsB, '/')))
    ) %>%
...
pca.variance <-
    summary(prcomp(assay(rld)))$importance["Proportion of Variance",
                                           c("PC1", "PC2")]
fig.pca <-
    fig.pca +
    labs(
        x = paste0('PC1: ', round(pca.variance['PC1']*100), '% variance'),
...
print(ggpubr::ggarrange(fig.pca.all, fig.pca,
                        nrow = 1, ncol = 2, align = "h"))
print(ggpubr::ggarrange(fig.count, fig.corr,
                        nrow = 1, ncol = 2, align = "h"))
...
for(i in 1:nrow(groups.AvsB.all.df)){
    ...
    volc <-
        ...
        geom_point_rast(
            aes(x = log2FC, y = -log10(padj), color = deType),
            size = 0.3
        ) +
        geom_vline(xintercept = c(0), color = "#990000", 
                   linetype = "dashed") +
        scale_color_manual(
    ...
    sca <-
        ...
        dplyr::left_join(
            ., dplyr::select(deseq2.res.AvsB, ID, deType),
            by = "ID"
        ) %>%
        ...
        labs(
            x = paste0("Intensity of ", conB),
            y = paste0("Intensity of ", conA),
            color = "deType"
        ) +
        scale_color_manual(
            values = c('down'= "#4d97cd", 'None'="#dee2e6", 'up'="#c74546")
    ...
    # 保存结果
    names(deseq2.res.AvsB) <- c(
        "ID", paste(c("log2FC", "padj", "deType"),
                    paste0(conA,'_vs_', conB),
                    sep = ".")
    )
    if(exists("deseq2.res")){
        deseq2.res <- left_join(
            deseq2.res, deseq2.res.AvsB, by = 'ID'
        )
    }else{
...
deseq2.res <- 
    cbind(deseq2.res, counts, counts.rlog[, -1]) %>% 
    tidyr::separate(col = ID, 
                    into = c('chr', 'start', 'end'), 
                    sep = '_', remove = F) %>% 
    dplyr::select(2,3,4,1, everything())
write.table(
    deseq2.res,
    paste0(outputFileName, ".xls"),
